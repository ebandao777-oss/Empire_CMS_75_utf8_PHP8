...
function EditNews($add,$userid,$username){
	...
	{
		$copyclassid=$add['copyclassid'];
		$cpcount=is_array($copyclassid)?count($copyclassid):0;
		if($cpcount)
		{
...
function InfoToDoc($add,$userid,$username){
	...
	//栏目
	$classid=$add['classid'];
	$count=is_array($classid)?count($classid):0;
	if($count)
	{
		for($i=0;$i<$count;$i++)
		{
