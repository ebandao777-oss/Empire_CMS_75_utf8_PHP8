...
if($ecms_config['sets']['webdebug']==2)
{
...
function eReturnSetGroups($groupid,$isnum=1){
    if(is_array($groupid)){$count=php8_count($groupid);}else{
        $count=0;
    }
	if($count==0)
	{
...
//字符截取函数
function sub($string,$start,$length,$mode=false,$dot='',$rephtml=0){
	global $ecms_config;
	if(!$start)$start=0;
...
function WriteFiletext($filepath,$string){
	...
	//jhc add for 8.0
	if(preg_match('#data/html/.*?php$#is',$filepath)){
	    $string=preg_replace_callback('#(\$[A-Za-z0-9_]+)\[([a-zA-Z_]+)\]#is',function($m){
	        return $m[1]."['".$m[2]."']";
	    },$string);
	}
	$fp=@fopen($filepath,"w");
...
//信息数统计加1
function DoUpdateAddDataNum($type,$stb,$addnum=1){
	global $empire,$dbtbpre;
	if(!$type)$type='info';
	if($type=='info')//信息
	{
...
function DoSafeCheckFromurl(){
	...
	$fromurl=$_SERVER['HTTP_REFERER']?$_SERVER['HTTP_REFERER']:null;
	if(!$fromurl)
	{
		return '';
	}
