...
function ReturnVote($votename,$votenum,$delvid,$vid,$enews=0){
	...
	if(empty($enews))
	{
		for($i=0;$i<is_array($votename)?count($votename):0;$i++)
		{
			//替换非法字符
...
function ReturnMorepicpathHtml($add){
	...
	$picpath='';
	$path_r=explode("\r\n",$add['morepic']);
	for($pj=0;$pj<php8_count($path_r);$pj++)
    {
		$p=$pj+1;
		if(($p-1)%$line==0||$p==1)
		{
...
//生成js
function GetNewsJs($classid,$line,$sub,$showdate,$enews,$tempr){
...
function hCheckEcmsRHash(){
	...
	if(isset($_GET[$rhashvar])&&$_GET[$rhashvar]==$rhash)
	{
	}
...
function hCheckEcmsEHash(){
	...
	if(isset($_GET[$ehashvar])&&$_GET[$ehashvar]==$ehash)
	{
	}
...
function ReturnAddCj($add,$cj,$do=0){
	...
	for($i=0;$i<php8_count($record_r)-1;$i++)
	{
...
function ReturnMorepicpath($smallpic,$bigpic,$picname,$delpicid,$picid,$add,$downurl,$down=0,$modtype=0,$fstb=1){
	...
	if(empty($down))
	{
		for($i=0;$i<php8_count($smallpic);$i++)
		{
	...
	else
	{
		for($i=0;$i<php8_count($smallpic);$i++)
		{
			//删除地址
			$del=0;
			for($j=0;$j<php8_count($delpicid);$j++)
			{
...
function ReturnDownpath($downname,$downpath,$delpathid,$pathid,$downuser,$fen,$thedownqz,$add,$foruser,$downurl,$down=0){
	...
	if(empty($down))
	{
		for($i=0;$i<php8_count($downname);$i++)
		{
			//替换非法字符
	...
	else
	{
		for($i=0;$i<php8_count($downname);$i++)
		{
			//删除下载地址
			$del=0;
			for($j=0;$j<is_array($delpathid)?count($delpathid):0;$j++)
			{
