...
//	$sql=$empire->query("insert into {$dbtbpre}enewsf(f,fname,fform,fhtml,fzs,isadd,isshow,iscj,cjhtml,myorder,ftype,flen,dotemp,tid,tbname,savetxt,fvalue,iskey,tobr,dohtml,qfhtml,isonly,linkfieldval,samedata,fformsize,tbdataf,ispage,adddofun,editdofun,qadddofun,qeditdofun,linkfieldtb,linkfieldshow,editorys,issmalltext,fmvnum) values('{$add['f']}','{$add['fname']}','{$add['fform']}','".eaddslashes2($fhtml)."','".eaddslashes($add['fzs'])."',1,1,$add['iscj'],'".eaddslashes2($cjhtml)."',$add['myorder'],'{$add['ftype']}','{$add['flen']}',1,$tid,'$tbname',$add['savetxt'],'".eaddslashes2($add['fvalue'])."',$add['iskey'],$add['tobr'],$add['dohtml'],'".eaddslashes2($qfhtml)."','{$add['isonly']}','".eaddslashes($add['linkfieldval'])."','{$add['samedata']}','{$add['fformsize']}','{$add['tbdataf']}','{$add['ispage']}','{$add['adddofun']}','{$add['editdofun']}','{$add['qadddofun']}','{$add['qeditdofun']}','{$add['linkfieldtb']}','{$add['linkfieldshow']}','{$add['editorys']}','{$add['issmalltext']}','{$add['fmvnum']}');");
	$sql = $empire->query("  
    INSERT INTO {$dbtbpre}enewsf (  
        f, fname, fform, fhtml, fzs, isadd, isshow, iscj, cjhtml, myorder,   
        ftype, flen, dotemp, tid, tbname, savetxt, fvalue, iskey, tobr,   
        dohtml, qfhtml, isonly, linkfieldval, samedata, fformsize, tbdataf,   
        ispage, adddofun, editdofun, qadddofun, qeditdofun, linkfieldtb,   
        linkfieldshow, editorys, issmalltext, fmvnum  
    ) VALUES (  
        '{$add['f']}', '{$add['fname']}', '{$add['fform']}', '".eaddslashes2($fhtml)."', '".eaddslashes($add['fzs'])."',   
        1, 1, {$add['iscj']}, '".eaddslashes2($cjhtml)."', {$add['myorder']},   
        '{$add['ftype']}', '{$add['flen']}', 1, $tid, '$tbname', '{$add['savetxt']}',   
        '".eaddslashes2($add['fvalue'])."', {$add['iskey']}, {$add['tobr']}, {$add['dohtml']},   
        '".eaddslashes2($qfhtml)."', {$add['isonly']}, '".eaddslashes($add['linkfieldval'])."',   
        '{$add['samedata']}', '{$add['fformsize']}', '{$add['tbdataf']}', '{$add['ispage']}',   
        '{$add['adddofun']}', '{$add['editdofun']}', '{$add['qadddofun']}', '{$add['qeditdofun']}',   
        '{$add['linkfieldtb']}', '{$add['linkfieldshow']}', '{$add['editorys']}', '{$add['issmalltext']}', '{$add['fmvnum']}'  
    );  
");
...
$classid=isset($classid)?$classid:null;
	...
	$sql = $empire->query("  
    UPDATE {$dbtbpre}enewsf   
    SET   
        f = '{$add['f']}',   
        fname = '{$add['fname']}',   
        fform = '{$add['fform']}',   
        fhtml = '".eaddslashes2($fhtml)."',   
        fzs = '".eaddslashes($add['fzs'])."',   
        iscj = {$add['iscj']},   
        cjhtml = '".eaddslashes2($cjhtml)."',   
        myorder = {$add['myorder']},   
        ftype = '{$add['ftype']}',   
        flen = '{$add['flen']}',   
        fvalue = '".eaddslashes2($add['fvalue'])."',   
        iskey = {$add['iskey']},   
        tobr = {$add['tobr']},   
        dohtml = {$add['dohtml']},   
        qfhtml = '".eaddslashes2($qfhtml)."',   
        isonly = '{$add['isonly']}',   
        linkfieldval = '{$add['linkfieldval']}',   
        samedata = '{$add['samedata']}',   
        fformsize = '{$add['fformsize']}',   
        ispage = '{$add['ispage']}',   
        adddofun = '{$add['adddofun']}',   
        editdofun = '{$add['editdofun']}',   
        qadddofun = '{$add['qadddofun']}',   
        qeditdofun = '{$add['qeditdofun']}',   
        linkfieldtb = '{$add['linkfieldtb']}',   
        linkfieldshow = '{$add['linkfieldshow']}',   
        editorys = '{$add['editorys']}',   
        issmalltext = '{$add['issmalltext']}',   
        fmvnum = '{$add['fmvnum']}'   
    WHERE fid = '{$add['fid']}'  
");
	TogSaveTxtF(1);//公共变量
...
//	$sql=$empire->query("update {$dbtbpre}enewsf set fname='{$add['fname']}',fform='{$add['fform']}',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add['fzs'])."',myorder=$add['myorder'],flen='{$add['flen']}',fvalue='".eaddslashes2($add['fvalue'])."',iskey=$iskey,qfhtml='".eaddslashes2($qfhtml)."',isonly='{$add['isonly']}',linkfieldval='{$add['linkfieldval']}',samedata='{$add['samedata']}',fformsize='{$add['fformsize']}',adddofun='{$add['adddofun']}',editdofun='{$add['editdofun']}',qadddofun='{$add['qadddofun']}',qeditdofun='{$add['qeditdofun']}',linkfieldtb='{$add['linkfieldtb']}',linkfieldshow='{$add['linkfieldshow']}'".$addupdate." where fid='$fid'");
    $sql=$empire->query("UPDATE {$dbtbpre}enewsf   
        SET   
            fname = '{$add['fname']}',  
            fform = '{$add['fform']}',  
            fhtml = '".eaddslashes2($fhtml)."',  
...
function TogMCj($cname,$cchange){
	...
	$field="<!--field--->";
	$c="";
	for($i=0;$i<php8_count($cchange);$i++)
	{
		$v=$cchange[$i];
...
function TogMqenter($cname,$cqenter){
	...
	$field="<!--field--->";
	$c="";
	for($i=0;$i<php8_count($cqenter);$i++)
	{
		$v=$cqenter[$i];
		$name=str_replace($field,"",$cname[$v]);
		$name=str_replace($record,"",$name);
...
function TogMSearch($cname,$schange){
	$c="";
	for($i=0;$i<php8_count($schange);$i++)
	{
		$v=$schange[$i];
		$c.=$v.",";
	}
...
function TogMustf($cname,$menter){
	$c="";
	for($i=0;$i<is_array($menter)?count($menter):0;$i++)
	{
		$v=$menter[$i];
		$c.=$v.",";
	}
...
$classid=isset($classid)?$classid:null;
	...
	$sql = $empire->query("  
    INSERT INTO {$dbtbpre}enewsmod (  
        mname, mtemp, mzs, cj, enter, tempvar, sonclass, searchvar, tid, tbname,   
        qenter, mustqenterf, qmtemp, listandf, setandf, listtempvar, qmname,   
        canaddf, caneditf, definfovoteid, showmod, usemod, myorder, orderf,   
        isdefault, listfile, printtempid, maddfun, meditfun, qmaddfun, qmeditfun  
    ) VALUES (  
        '{$add['mname']}', '" . eaddslashes2($add['mtemp']) . "', '{$add['mzs']}',   
        '$cj', '$enter', '$tempvar', '', '$searchvar', $tid, '$tbname',   
        '$qenter', '$mustqenterf', '" . eaddslashes2($add['qmtemp']) . "',   
        '" . addslashes($listandf) . "', '$setandf', '$listtempvar', '{$add['qmname']}',   
        '$canaddf', '$caneditf', {$add['definfovoteid']}, '$showmod', '$usemod',   
        '$myorder', '$orderf', 0, '$listfile', '{$add['printtempid']}',   
        '{$add['maddfun']}', '{$add['meditfun']}', '{$add['qmaddfun']}', '{$add['qmeditfun']}'  
    )  
");  
	$mid=$empire->lastid();
...
//	$sql=$empire->query("update {$dbtbpre}enewsmod set mname='{$add['mname']}',mtemp='".eaddslashes2($add['mtemp'])."',mzs='{$add['mzs']}',cj='$cj',enter='$enter',tempvar='$tempvar',searchvar='$searchvar',qenter='$qenter',mustqenterf='$mustqenterf',qmtemp='".eaddslashes2($add['qmtemp'])."',listandf='".addslashes($listandf)."',setandf=$setandf,listtempvar='$listtempvar',qmname='{$add['qmname']}',canaddf='$canaddf',caneditf='$caneditf',definfovoteid=$add['definfovoteid'],showmod='$showmod',usemod='$usemod',myorder='$myorder',orderf='$orderf',listfile='$listfile',printtempid='{$add['printtempid']}',maddfun='{$add['maddfun']}',meditfun='{$add['meditfun']}',qmaddfun='{$add['qmaddfun']}',qmeditfun='{$add['qmeditfun']}' where mid='{$add['mid']}'");
    $sql = $empire->query("  
    UPDATE {$dbtbpre}enewsmod  
    SET   
        mname = '{$add['mname']}',  
        mtemp = '" . eaddslashes2($add['mtemp']) . "',  
        mzs = '{$add['mzs']}',  
        cj = '$cj',  
        enter = '$enter',  
        tempvar = '$tempvar',  
        searchvar = '$searchvar',  
        qenter = '$qenter',  
        mustqenterf = '$mustqenterf',  
        qmtemp = '" . eaddslashes2($add['qmtemp']) . "',  
        listandf = '" . addslashes($listandf) . "',  
        setandf = $setandf,  
        listtempvar = '$listtempvar',  
        qmname = '{$add['qmname']}',  
        canaddf = '$canaddf',  
        caneditf = '$caneditf',  
        definfovoteid = {$add['definfovoteid']},  
        showmod = '$showmod',  
        usemod = '$usemod',  
        myorder = '$myorder',  
        orderf = '$orderf',  
        listfile = '$listfile',  
        printtempid = '{$add['printtempid']}',  
        maddfun = '{$add['maddfun']}',  
        meditfun = '{$add['meditfun']}',  
        qmaddfun = '{$add['qmaddfun']}',  
        qmeditfun = '{$add['qmeditfun']}'  
    WHERE mid = '{$add['mid']}'  
");
	//更新表单
